...
var RESTSerializer = JSONSerializer.extend({
  ...
  init: function() {
    ...
    if (this.get('isNewSerializerAPI')) {
      this.normalizeArray = this.newNormalizeArray;
      this.pushPayload = this.newPushPayload;
    }
  },
  ...
  */
  ...
  newNormalize: function(modelClass, resourceHash, prop) {
    if (this.normalizeHash && this.normalizeHash[prop]) {
      Ember.deprecate('Using `normalizeHash` has been deprecated. Please use `normalize` instead.');
  ...
  extractRelationship: function(modelClass, relationshipHash) {
    ...
    if (relationship && relationship.type) {
      relationship.type = this.modelNameFromPayloadKey(relationship.type);
    }
    return relationship;
  },
  ...
  newPushPayload: function(store, rawPayload) {
    let document = {
      data: [],
      included: []
    };
  ...
  newNormalizeArray: function(store, modelName, arrayHash, prop) {
    let document = {
      data: [],
      included: []
    };
    ...
    let serializer = store.serializerFor(modelClass);

    /*jshint loopfunc:true*/
    forEach.call(arrayHash, function(hash) {
